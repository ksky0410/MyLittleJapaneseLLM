...
PROJECT = Path("/content/small_llm_078")
CONDITION = "issue1-both-50m-sft-from-5m-seed123-3k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp078-lightweight.tar.gz")
MANIFEST = Path("/content/exp078-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験078の軽量成果物が見つかりません")
