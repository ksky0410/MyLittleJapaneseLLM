"""実験052の20M rehearsal ratio sweepをColab T4で順に実行する。"""
...
CONDITIONS = (("ratio010", "0.10"), ("ratio050", "0.50"))
...
def main() -> None:
    ...
    print(
        json.dumps(
            {"experiment": "052", "project": str(PROJECT), "input_hashes": hashes},
            ensure_ascii=False,
            indent=2,
        ),
        flush=True,
    ...
    for name, ratio in CONDITIONS:
        output_name = f"issue1-both-20m-rehearsal-{name}-colab-1k"
        command = [
            sys.executable,
            ...
            "--samples-dir",
            f"artifacts/samples/{output_name}",
            "--rehearsal-tokens",
            REHEARSAL,
            "--rehearsal-ratio",
            ratio,
        ...
        print(
            json.dumps(
                {"experiment": "052", "condition": name, "ratio": ratio},
                ensure_ascii=False,
            ),
            flush=True,
        )
