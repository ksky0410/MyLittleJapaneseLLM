"""実験081の軽量成果物とbest checkpointをColab側でarchive化する。"""
...
PROJECT = Path("/content/small_llm_081")
CONDITION = "issue1-both-50m-pretrain-5m-5k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp081-lightweight.tar.gz")
BEST_ARCHIVE = Path("/content/exp081-best-checkpoint.tar")
MANIFEST = Path("/content/exp081-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験081の軽量成果物が見つかりません")
