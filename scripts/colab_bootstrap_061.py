"""実験061のrehearsal ratio 0.25/0.75比較をColab T4で実行する。"""
...
BUNDLE = Path("/content/exp061_bundle.tar.gz")
...
CONDITIONS = {
    "ratio025": ("0.25", "issue1-056base-rehearsal-ratio025-eos050-colab-6k-fixedlr3k"),
    "ratio075": ("0.75", "issue1-056base-rehearsal-ratio075-eos050-colab-6k-fixedlr3k"),
}

EXPECTED_FILES = {
    CONFIG: "fbc654df1a77c4847524de87f295341641753435693e13dd6d45aaa9f66d3729",
    "scripts/train_sft_torch.py": "551b4353933be9d590ac7172d612075900ffc92a8362aa9865297e20da9a1d39",
...
def main() -> None:
    ...
    if mismatches:
        raise RuntimeError(
            "061 bundleの入力または実行コードhashが一致しません: "
            + json.dumps(mismatches, ensure_ascii=False)
        )
    for condition, (ratio, output_name) in CONDITIONS.items():
        command = [
            sys.executable,
            ...
            "--rehearsal-ratio", ratio,
            "--eos-loss-weight", "0.50",
        ...
        print(json.dumps({"experiment": "061", "condition": condition, "rehearsal_ratio": float(ratio), "steps": 6000, "lr_schedule_steps": 3000}, ensure_ascii=False), flush=True)
