...
PROJECT = Path("/content/small_llm_077")
CONDITION = "issue1-both-50m-sft-from-5m-long025-3k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp077-lightweight.tar.gz")
MANIFEST = Path("/content/exp077-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験077の軽量成果物が見つかりません")
