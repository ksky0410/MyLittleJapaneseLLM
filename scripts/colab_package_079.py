...
CONDITION = "issue1-both-50m-sft-from-5m-long025-seed123-3k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp079-lightweight.tar.gz")
MANIFEST = Path("/content/exp079-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験079の軽量成果物が見つかりません")
