"""実験089の軽量成果物とbest checkpointをColab側でarchive化する。"""
...
CONDITION = "issue1-both-50m-pretrain-5m-continuation-5k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp089-lightweight.tar.gz")
BEST_ARCHIVE = Path("/content/exp089-best-checkpoint.tar")
MANIFEST = Path("/content/exp089-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験089の軽量成果物が見つかりません")
    ...
    manifest = {
        "experiment": "089", "condition": CONDITION,
