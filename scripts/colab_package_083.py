"""実験083の軽量成果物とbest checkpointをColab側でarchive化する。"""
...
PROJECT = Path("/content/small_llm_083")
CONDITION = "issue1-both-50m-sft-from-5m-two-pass-seed123-eos0-3k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp083-lightweight.tar.gz")
BEST_ARCHIVE = Path("/content/exp083-best-checkpoint.tar")
MANIFEST = Path("/content/exp083-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験083の軽量成果物が見つかりません")
