"""実験085の軽量成果物とbest checkpointをColab側でarchive化する。"""
...
CONDITION = "issue1-both-50m-sft-from-5m-two-pass-seed123-10k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp085-lightweight.tar.gz")
BEST_ARCHIVE = Path("/content/exp085-best-checkpoint.tar")
MANIFEST = Path("/content/exp085-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験085の軽量成果物が見つかりません")
