"""実験088の軽量成果物とbest checkpointをColab側でarchive化する。"""
...
CONDITION = "issue1-both-50m-sft-from-5m-two-pass-seed123-10k-770k-each-long0125"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp088-lightweight.tar.gz")
BEST_ARCHIVE = Path("/content/exp088-best-checkpoint.tar")
MANIFEST = Path("/content/exp088-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験088の軽量成果物が見つかりません")
    ...
    manifest = {
        "experiment": "088", "condition": CONDITION,
