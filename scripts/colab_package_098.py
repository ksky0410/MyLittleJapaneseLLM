"""実験098のColab成果物を軽量archiveとhash manifestへまとめる。"""
...
CONDITION = "issue1-both-50m-pretrain-20m-40k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp098-lightweight.tar.gz")
MANIFEST = Path("/content/exp098-manifest.json")
BEST_ARCHIVE = Path("/content/exp098-best-checkpoint.tar")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験098の軽量成果物が見つかりません")
