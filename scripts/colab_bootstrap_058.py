"""実験058のEOS loss weight 0.75 SFTをColab T4で実行する。"""
...
PROJECT = Path("/content/small_llm_058")
...
OUTPUT = "issue1-056base-rehearsal-ratio050-eos075-colab-3k"
...
def main() -> None:
    ...
    if mismatches:
        raise RuntimeError(
            "058 bundleの入力または実行コードhashが一致しません: "
    ...
    command = [
        ...
        "--eos-loss-weight", "0.75",
        "--sample-template", "conversation",
        "--sample-speaker-a", "DA",
        "--sample-speaker-b", "DC",
        "--device", "auto",
    ]
    print(json.dumps({"experiment": "058", "condition": "eos075", "eos_loss_weight": 0.75, "verified_files": len(EXPECTED_FILES), "steps": 3000}, ensure_ascii=False), flush=True)
