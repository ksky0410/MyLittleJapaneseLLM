...
CONDITION = "issue1-both-50m-sft-from-5m-rehearsal020-3k"
CHECKPOINT_ROOT = PROJECT / "artifacts/checkpoints" / CONDITION
SAMPLES_ROOT = PROJECT / "artifacts/samples" / CONDITION
ARCHIVE = Path("/content/exp076-lightweight.tar.gz")
MANIFEST = Path("/content/exp076-manifest.json")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError("実験076の軽量成果物が見つかりません")
