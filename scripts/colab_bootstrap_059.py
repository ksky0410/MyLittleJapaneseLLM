"""実験059のEOS loss weight 0.50・6,000 step SFTをColab T4で実行する。"""
...
BUNDLE = Path("/content/exp059_bundle.tar.gz")
PROJECT = Path("/content/small_llm_059")
...
OUTPUT = "issue1-056base-rehearsal-ratio050-eos050-colab-6k"
...
def main() -> None:
    ...
    if mismatches:
        raise RuntimeError(
            "059 bundleの入力または実行コードhashが一致しません: "
    ...
    print(json.dumps({"experiment": "059", "condition": "eos050-6k", "eos_loss_weight": 0.50, "steps": 6000}, ensure_ascii=False), flush=True)
