"""実験060の学習率曲線固定・EOS loss weight 0.50 SFTをColab T4で実行する。"""
...
BUNDLE = Path("/content/exp060_bundle.tar.gz")
PROJECT = Path("/content/small_llm_060")
...
OUTPUT = "issue1-056base-rehearsal-ratio050-eos050-colab-6k-fixedlr3k"
...
def main() -> None:
    ...
    if mismatches:
        raise RuntimeError(
            "060 bundleの入力または実行コードhashが一致しません: "
    ...
    print(json.dumps({"experiment": "060", "condition": "eos050-6k-fixedlr3k", "eos_loss_weight": 0.50, "steps": 6000, "lr_schedule_steps": 3000}, ensure_ascii=False), flush=True)
