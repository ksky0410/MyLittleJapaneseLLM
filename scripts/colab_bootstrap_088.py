"""実験088の長文応答12.5%層化SFTをColab GPUで実行する。"""
...
BUNDLE = Path("/content/exp088_bundle_reassembled.tar.gz")
PROJECT = Path("/content/small_llm_088")
CONFIG = "configs/issue1-both-50m-sft-from-5m-two-pass-seed123-10k-770k-each-long0125.toml"
...
OUTPUT = "issue1-both-50m-sft-from-5m-two-pass-seed123-10k-770k-each-long0125"
...
def main() -> None:
    ...
    if missing or mismatches:
        raise RuntimeError(
            "088 bundleの入力検証に失敗しました: "
            + json.dumps({"missing": missing, "mismatches": mismatches}, ensure_ascii=False)
        )
    print(json.dumps({
        "experiment": "088",
        "project_dir": str(PROJECT),
        "condition": "both-50m-sft-770k-each-rehearsal020-long0125-seed123-10k",
        "verified_inputs": len(required),
        "max_steps": 10000,
        "long_response_ratio": 0.125,
