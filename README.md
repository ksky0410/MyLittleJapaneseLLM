...
このプロジェクトの最終的な目標は、強い教師モデルの蒸留に頼らず、データの質と量、学習の反復、モデル構造、SFTの形式、評価方法を改善することで、自作の小型日本語LLMを少しずつ強くし、自然な日本語で会話できるようにすることです。Tokenizerや長文比率の比較は、この目標へ近づくための診断実験として扱います。

当面は、50Mモデルを基準に、一般日本語・Issue #1の会話・医師国家試験由来データを混ぜたコーパスを増やし、同じデータを何周学習させるか、データの品質とsource比率をどうするか、context lengthをどこまで伸ばすか、pretrainingとSFTをどの順序・割合で組み合わせるかを追究します。評価ではlossだけでなく、固定chat-test、人手レビュー、生成全文を合わせて確認し、モデルが短い定型句を返すだけでなく、履歴を保ち、話題に沿った自然な応答を返せるかを見ます。

蒸留や教師LLMによるreasoningデータは、将来の比較対象として記録することはありますが、主線の成功条件には含めません。主線では、公開データとこのリポジトリ内の前処理・学習コードだけで性能がどこまで伸びるかを検証します。
