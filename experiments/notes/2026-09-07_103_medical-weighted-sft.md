...
### 2026-09-07：累積3,000 stepの本走

パイロットの最良checkpointから、`--start-step 1000` を指定して累積step 3,000まで継続した。実行時間は161.21秒、ピークGPUメモリは約1.49GBだった。NaN、OOM、shape errorは発生しなかった。実験中のvalidation lossはstep 1,250の2.898176からいったん2.903777まで悪化した後、step 3,000で2.890588まで改善した。混合validationの最良checkpointはstep 3,000で、重みSHA-256は `c90d9e3237119f11c37a39ed31a2db40753b7bbbd6812d146587a2bef325546a` である。

領域別lossはFineWeb 2.959577、一般 4.089108、会話 2.101740、医療 1.987456だった。実験102と比べると一般lossは4.089575からわずかに改善したが、会話lossは2.100814からほぼ同じ範囲、医療lossは1.987503からほぼ同じ範囲であり、医療比率を4倍にした効果はloss上では明確ではなかった。

一般会話48例ではEOS 48/48、平均生成7.44 tokens、token overlap F1 0.1811だった。実験102のEOS 48/48、平均7.90 tokens、F1 0.1892から少し下がったものの、終了挙動は維持した。医療162例ではEOS 158/162、平均生成56.73 tokens、F1 0.3797となった。回答形式は162/162例で抽出できたが、正解は28例、正解率17.28%にとどまり、パイロットの39/162、24.07%と実験102の37/161、22.98%を下回った。

この結果から、医療QAを4倍にしたまま学習stepを増やすと医療正解率が単調に上がるわけではなく、混合validation lossを改善するcheckpointが医療問題の正解率を保証しないことが分かった。今回の本走checkpointは「最も賢い医療QAモデル」として採用しない。現時点の医療QA用途の候補は、同じ実験のstep 1,000パイロットcheckpointである。ただし、24.07%も5択問題の偶然水準に近く、十分な医学的能力とは判断しない。

本実験の主な成果は、データ比率の変更だけでは正答率が改善しないことと、SFT中のcheckpointを混合validationだけで選ぶべきではないことを確認した点にある。次は、同じ評価問題を学習に使わないまま、医療正解率・一般会話のEOS・一般validation lossを含む複合評価でcheckpointを選ぶ仕組みを追加する。そのうえで、医療QAの反復倍率をさらに上げる前に、医療回答の説明部分を短く安定させたデータと、一般会話データを同じ学習stepで混ぜる条件を比較する。教師LLMによる蒸留はまだ使わない。
