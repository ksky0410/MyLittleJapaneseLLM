...
step 8100はvalidation loss 3.078655、PPL 21.7291、経過10036.61秒、step 8200は3.077182、PPL 21.6972、経過10154.50秒、step 8300は3.070465、PPL 21.5519、経過10274.53秒でした。step 8400は3.070371、PPL 21.5499、経過10395.34秒でbestを更新しました。step 8500は3.071702、PPL 21.5786、経過10516.66秒、step 8600は3.065273、PPL 21.4403、経過10638.29秒でbestを更新しました。step 8700は3.070584、PPL 21.5545、経過10759.40秒、step 8800は3.066386、PPL 21.4642、経過10877.39秒、step 8900は3.058493、PPL 21.2954、経過10997.16秒でbestを更新しました。step 9000はtrain loss 2.518794、SFT loss 2.408636、rehearsal loss 2.959426、validation loss 3.054313、PPL 21.2066、経過11118.33秒でした。

step 9100はvalidation loss 3.054842、PPL 21.2178、経過11242.00秒、step 9200は3.048854、PPL 21.0912、経過11360.57秒でbestを更新しました。step 9300は3.051755、PPL 21.1524、経過11480.51秒、step 9400は3.051861、PPL 21.1547、経過11599.49秒、step 9500は3.051132、PPL 21.1393、経過11717.47秒、step 9600は3.050044、PPL 21.1163、経過11840.48秒でbestを更新しました。step 9700は3.048331、PPL 21.0801、経過11960.01秒、step 9800は3.049731、PPL 21.1097、経過12080.48秒、step 9900は3.050696、PPL 21.1300、経過12198.67秒でした。最終step 10000はtrain loss 2.811664、SFT loss 2.690283、rehearsal loss 3.297189、validation loss 3.047611、PPL 21.0650、経過12317.08秒で、summary上の総経過時間は12322.09秒でした。step 10000まで完走し、best checkpointはstep 10000です。MPS上でOOM、NaN、shape error、checkpoint保存エラーは発生しませんでした。

step 10000時点で周期checkpointは`step_010000.pt`一個、bestは`best.pt`一個だけです。best重みのSHA-256は`6bbb0ff0fac6c63ce9c3a4fc807744072390bad8f7daaa35066016901fa4180f`、best metadataは`e887389dc067a12a09954e210b2562c06ed1f7c888f2863e4b432ed5a1301470`、summaryは`652e38e676c940164d61cc2997b7141382d1b474f38f4b587fe8f33a526c5e2c`です。これから最終固定prompt、held-out chat、5領域validationを実行します。

## 実験終了後の結果と解釈

最終best checkpointはstep 10,000です。Issue #1固定promptのconversation評価では8例すべてがEOSへ到達しましたが、`まじで→こんにちは`、`それな→こんにちはー!`、`今日なにしてた？→こんばんは!`、`やば→こんにちは`、`なんかさ→おはようございます!`、`いやそれは→こんにちは!`、`おつかれ→こんにちは`、`明日ひま？→こんにちは!`となりました。入力に応じた自然な応答は確認できず、step 4,900の中間評価から挨拶縮退は解消しませんでした。最終評価JSONのSHA-256は`e32d33e93bdd46001543a2361693aca959b0902a0c4587514a1b417dc4530cff`、テキストのSHA-256は`7e583e3369894f5539757a6c15cc3ed59a3794a5de477979ecf84ef903775240`です。

48例のheld-out chat-testでは、48例すべてがEOSへ到達し、平均生成長は7.75 token、全体のtoken overlap F1は0.243491でした。shortはF1 0.434288、mediumは0.160495、longは0.135690でした。実験087の0.216545、実験085の0.236752、実験092の0.212413を全体F1で上回り、short・medium・longの全層でも087を上回りました。最終評価JSONのSHA-256は`219165049d152e9ac01e35490575ede288a473f75aac0722fa7da434abe2c3e6`、テキストのSHA-256は`6b4b08ecda009f5d1a249f76a7b4f4d75e94cde22787a2ec69354454e4fc209a`です。なお、このF1は参照応答とのtoken overlapであり、自然さや意味の正しさを完全には測りません。また、093は10,000 step完走、092はstep 7,400で保存失敗したcheckpointとの比較であるため、092との差はbackendと学習stepの違いを含みます。

5領域validationでは、general loss 4.341535（PPL 76.8254）、conversation loss 2.412162（PPL 11.1581）、medical loss 2.509977（PPL 12.3046）、RPC loss 2.371365（PPL 10.7120）、MRMP loss 1.992494（PPL 7.3338）でした。step 7,400の092と比較すると、generalは4.352421から0.010886、conversationは2.437993から0.025830、medicalは2.530518から0.020541、RPCは2.394490から0.023125、MRMPは2.028177から0.035683改善しています。評価JSONのSHA-256は`b9d3ea369cec8f9ab0535f2fe7f155dc24d1d95556aac0b21618115cf5e29773`です。

事前の仮説は部分的に支持されました。応答機能をsource別に選別したSFTは、validationとheld-out chat-testの両方を改善し、単純なresponse token増加や長文比率固定より良い結果になりました。一方で、短いIssue #1固定promptへの入力依存応答は改善しませんでした。この差から、093は会話データの分布への汎化を改善したものの、単独の短い口語promptに対して適切な発話機能を選ぶ能力までは学習できていないと解釈します。固定promptの8例は学習分布内の典型的な会話履歴ではなく、held-out F1だけで自然な会話能力を主張しない方針を維持します。

093の生成サンプル、metrics、checkpoint metadata、評価JSON、評価本文はGitHubへ保存します。重い`.pt`本体はbestと最新周期checkpointをローカルに保持し、GitHubには追加しません。学習終了後の空き容量は約2.5GiBで、step 10,000の保存処理は成功しました。

## 次に試すこと

094では、093の選別方式をそのまま捨てず、固定promptの挨拶縮退だけを対象に切り分けます。まずheld-out評価の入力履歴長、source、カテゴリ、train overlapを分解し、F1改善がどの種類の会話から生じたかを確認します。その後、Issue #1の8 promptに対応する実際のRPC・MRMP発話を検索し、同じ短い口語表現を含む候補が十分に選ばれているかを監査します。外部教師LLMの蒸留は使わず、既存コーパスからの選別または少量の人手作成データだけを候補にします。

093は、一般validationとheld-out chat-testでは現時点の最良候補として保存しますが、固定promptで自然な応答が出ないため、完成モデルとしては採用しません。094では評価分布の監査を先に行い、必要なら短い口語応答への重み付けを一変数だけ変更して比較します。
