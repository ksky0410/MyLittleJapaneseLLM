...
50Mの最良checkpoint（step 1,300）を、20Mの`issue1-both-20m-colab-2p5k`基盤（step 1,700）と同じ評価スクリプトおよびeval batches 20で比較しました。20Mのparameter数は19,308,032、50Mは50,207,616です。領域別validation lossは、generalが20Mの6.214521に対して50Mは6.228799（差+0.014278）、conversationが3.250831に対して3.301992（+0.051161）、medicalが3.354361に対して3.522565（+0.168204）、RPCが3.214423に対して3.273876（+0.059452）、MRMPが2.761618に対して2.797225（+0.035608）でした。5領域すべてで50Mが悪化しており、同じ約1M Tokenを約5.12M exposure Tokenで学習する条件では、容量拡張による改善は確認できませんでした。約1M Tokenに対して50Mモデルを学習すると、20Mより早くデータ不足または過学習の影響を受けるという仮説を支持します。

chat-testは50Mで48/48件がEOSへ到達し、平均生成長8.396 Token、overall token-overlap F1 0.072697でした。20M基盤の48件中47件EOS、平均10.938 Token、F1 0.059590と比べると、全体F1は+0.013107、shortは0.065855から0.094900、mediumは0.054617から0.073505へ改善し、longは0.058298から0.049686へ悪化しました。これは領域別lossの悪化と一致しないため、50Mの会話能力が総合的に向上したとは解釈せず、EOS到達率と短い語句の表面一致がF1を押し上げた可能性を残します。50Mの領域評価は[`issue1-both-50m-pretrain-mps-2p5k-domains.json`](../../artifacts/evaluations/issue1-both-50m-pretrain-mps-2p5k-domains.json)、chat評価は[`issue1-both-50m-pretrain-mps-2p5k-chat-test-v1.json`](../../artifacts/evaluations/issue1-both-50m-pretrain-mps-2p5k-chat-test-v1.json)と[`TXT出力`](../../artifacts/evaluations/issue1-both-50m-pretrain-mps-2p5k-chat-test-v1.txt)、人手レビュー用テンプレートは[`issue1-both-50m-pretrain-mps-2p5k-chat-review.json`](../../artifacts/evaluations/issue1-both-50m-pretrain-mps-2p5k-chat-review.json)です。レビュー状態は未確認であり、生成の自然さ・話題適合性・会話継続性については人手評価待ちです。

## 次に試すこと

次は、この50Mのbest checkpointへ実験067相当の標準response-only SFTとrehearsal ratio 0.20、EOS loss weight 0.50を適用し、20M SFTとの容量差を比較します。同時に、今回と同じ50Mモデルへデータ量を増やした事前学習条件を別実験として設定し、「容量を増やす」効果と「データを増やす」効果を分離します。人手レビューでは自動F1だけでなく、自然さ、話題適合性、応答の完結性、不要な反復を確認します。MPSの速度またはメモリが厳しい場合は、Colab T4が復旧した時点で同じconfigを再実行できるようにします。
