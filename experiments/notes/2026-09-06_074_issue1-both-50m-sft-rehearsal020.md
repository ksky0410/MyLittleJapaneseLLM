...
最終best checkpointをCPUでeval batches 20の共通評価へかけ、20Mの実験067相当best（step 2,900、19,308,032 parameters）と比較しました。50Mの領域別validation lossはgeneral 5.364814、conversation 2.910169、medical 3.278567、RPC 2.906686、MRMP 2.249693でした。20Mはそれぞれ5.469881、2.897588、3.218233、2.899929、2.341413でした。差分（50M−20M）はgeneral -0.105066、conversation +0.012580、medical +0.060335、RPC +0.006757、MRMP -0.091720です。50MはgeneralとMRMPでは改善しましたが、conversation・medical・RPCでは僅かに悪化しました。したがって、50M化が全領域でSFT性能を改善したとは言えず、事前学習時に見えたデータ量不足の影響が一部領域へ残っている可能性があります。

固定chat-testは50Mで48/48件EOS、平均生成長7.979 Token、overall token-overlap F1 0.220457でした。20Mは48/48件EOS、平均10.479 Token、F1 0.207753でした。差分はoverall F1 +0.012703、平均生成長 -2.500 Tokenです。層別F1はshortが20Mの0.322669から50Mの0.371271へ+0.048602、mediumが0.166843から0.170805へ+0.003962、longが0.133749から0.119294へ-0.014455でした。短い挨拶・相づちの表面一致は改善しましたが、longの一致率と生成長は悪化しているため、50Mで長い会話の話題継続が改善したとは判断しません。領域評価は[`issue1-both-50m-sft-source-rehearsal020-mps-3k-domains.json`](../../artifacts/evaluations/issue1-both-50m-sft-source-rehearsal020-mps-3k-domains.json)、chat評価は[`issue1-both-50m-sft-source-rehearsal020-mps-3k-chat-test-v1.json`](../../artifacts/evaluations/issue1-both-50m-sft-source-rehearsal020-mps-3k-chat-test-v1.json)と[`TXT全文`](../../artifacts/evaluations/issue1-both-50m-sft-source-rehearsal020-mps-3k-chat-test-v1.txt)、人手レビュー用テンプレートは[`issue1-both-50m-sft-source-rehearsal020-mps-3k-chat-review.json`](../../artifacts/evaluations/issue1-both-50m-sft-source-rehearsal020-mps-3k-chat-review.json)です。レビュー状態は未確認であり、自然さ・話題適合性・応答完結性は人手評価待ちです。

## 次に試すこと

50Mのratio 0.20 SFTは完走し、短い応答の自動F1は20Mを上回りましたが、long応答と一部領域lossは改善しませんでした。次は、同じ50M構造でpretraining Token数を増やし、今回のデータ不足仮説を直接検証します。SFT側では、長い応答を一定割合で含める条件、またはSFTとrehearsalのToken予算をgradient accumulationで独立に制御する条件を比較します。人手レビューが完了するまでは長文samplingの採否を決めず、自然さ・話題適合性・反復・無関係な話題への逸脱を確認します。Colabが安定して利用できる場合は、次の長時間pretrainingや50M以上のモデルへ移行し、同じノートと成果物管理を維持します。
